...
const FoodDetails: React.FC = () => {
  ...
  useEffect(() => {
    async function loadFood(): Promise<void> {
      api.get(`/foods/${routeParams.id}`).then(response => {
        ...
        const formattedPrice = formatValue(foundFood.price);
        const formattedFood = Object.assign(foundFood, { formattedPrice });

        const foodExtras = foundFood.extras.map((extra: Extra) => {
      ...
      api.get<Food[]>('/favorites').then(response => {
        const foundFavorite: Food[] = response.data.filter(
  ...
  function handleDecrementExtra(id: number): void {
    ...
    extrasList[extraIndex].quantity === 0
      ? 0
      : (extrasList[extraIndex].quantity -= 1);
  ...
  const toggleFavorite = useCallback(() => {
    ...
    if (!isFavorite) {
      api.post<Food[]>('/favorites', food).then(() => {
        setIsFavorite(true);
      });
    }
  }, [isFavorite, food]);
  ...
  const cartTotal = useMemo(() => {
    ...
    const totalOrder = formatValue(
      extrasTotalPrice + food.price * foodQuantity,
    );
